xgbenc_param_2 <- function(x.train, best_param, nround, fold, nthreads){
...
###
grad[i] <- (exp_p * rk - as.numeric(y>0)) - lambda2*alpha2*g_beta/length(beta) - lambda2*(1-alpha2)*beta/length(beta)
hess[i] <- (exp_p * rk - exp_p*exp_p*sk ) - lambda2*(1-alpha2)*beta/(length(beta)*length(beta))
last_abs_y = abs_y
last_exp_p = exp_p
}
grad=grad[ran]		
...
lambda_1 = c(0.00001,0.0001,0.001,0.01,0.1,1,2,5,10)     # 初步确定的λ
alpha_1 = c(0.00001,0.0001,0.001,0.01,0.1,1,2,5,10)        # 初步确定的α

for (lambda2 in lambda_1){
for (alpha2 in alpha_1){            # α嵌套循环
  print (lambda2)
write(lambda2,file=sprintf("LDL_%d.txt",fold),append=TRUE,sep=" ")
  print (alpha2)	               # 打印此时的α
write(alpha2,file=sprintf("LDL_%d.txt",fold),append=TRUE,sep=" ")
  set.seed(4321)
  ...
  print(cindex)	              # 打印此时的cindex
write(cindex,file=sprintf("LDL_%d.txt",fold),append=TRUE,sep=" ")
  try(
  if (best_cindex<cindex){
   best_cindex=cindex
   best_lambda=lambda2
   best_alpha=alpha2              # 记录初步的α
   })
  }
}

print(best_lambda)
write(best_lambda,file=sprintf("LDL_%d.txt",fold),append=TRUE,sep=" ")
print(best_alpha)                    # 打印初步确定的α
write(best_alpha,file=sprintf("LDL_%d.txt",fold),append=TRUE,sep=" ")

lambda_2 = c(seq(best_lambda/5,best_lambda,best_lambda/10),seq(best_lambda*2,best_lambda*10,best_lambda))	# 细调λ
alpha_2 = c(seq(best_alpha/5,best_alpha,best_alpha/10),seq(best_alpha*2,best_alpha*10,best_alpha))      # 细调α

for (lambda2 in lambda_2){
for (alpha2 in alpha_2){          # α嵌套循环
  print (lambda2)
write(lambda2,file=sprintf("LDL_%d.txt",fold),append=TRUE,sep=" ")
  print (alpha2)	             # 打印此时的α
write(alpha2,file=sprintf("LDL_%d.txt",fold),append=TRUE,sep=" ")
...
print(best_lambda)
write(best_lambda,file=sprintf("LDL_%d.txt",fold),append=TRUE,sep=" ")
print(best_alpha)	            # 打印最佳的α
write(best_alpha,file=sprintf("LDL_%d.txt",fold),append=TRUE,sep=" ")
print(best_cindex)
write(best_cindex,file=sprintf("LDL_%d.txt",fold),append=TRUE,sep=" ")
write("********************",file=sprintf("LDL_%d.txt",fold),append=TRUE,sep=" ")
write("********************",file=sprintf("LDL_%d.txt",fold),append=TRUE,sep=" ")
...
save("result", file = sprintf("%d_xgbenc_result.RData", fold))
